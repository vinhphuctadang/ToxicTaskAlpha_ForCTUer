...
namespace ToxicTask_Alpha1
{
    public partial class Form1 : Form
    {
        ...
        Dictionary<String, List<SubjectClass>> database = null;
        ...
        private void OnLoadDatabase(String excelFileDirectory)
        {
            ...
            while (rowIndex <= targetIndex)
            {
                ...
                if (!prevClassID.Equals(classCode))
                {
                    ...
                    focusSubjectClass = new SubjectClass(openSubjectClassData[rowIndex, 2].ToString(), openSubjectClassData[rowIndex, 11].ToString(),
            ...
            for (int i = 0; i < subjects.RowCount; ++i)
            {
                ...
                if (subjects[0, i].Value != null)
                    ...
                    else
                    {
                        ...
                        subjects[1, i].Value = resultValue[0].getName();
                    }
        ...
        private void onPlanInput(object sender, DataGridViewCellEventArgs e)
        {
            var value = ((DataGridView)sender)[e.ColumnIndex, e.RowIndex].Value.ToString();

            List<SubjectClass> tmp;

            if (database.TryGetValue(value, out tmp))
            {
                ((DataGridView)sender)[1, e.RowIndex].Value = tmp[0].getName();
            }
            else
            {
                MessageBox.Show("Lỗi: Mã học phần " + value + " không tồn tại");
                ((DataGridView)sender)[0, e.RowIndex].Value = ((DataGridView)sender)[1, e.RowIndex].Value = "";
            }
        }

        private void subjects_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
            if (e.ColumnIndex == 0)
    ...
    public class SubjectClass
    {
        ...
        public SubjectClass(String code, String name, String id)
        {
        ...
        public override string ToString()
        {
            return name + " " + id + Environment.NewLine + code + Environment.NewLine;
        }
